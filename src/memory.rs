...
impl MemoryUnit {
        ...
        match addr {
            ...
            0x8000..=0x9FFF => {
                if self.ppu_mode != 3 || source == RequestSource::PPU {
                    if self.vram_bank == 0 {
                        self.vram_0[addr - 0x8000]
                    } else {
                        self.vram_1[addr - 0x8000]
                    }
                } else {
                    0xFF
                }
            ...
            0xC000..=0xCFFF => self.internal_ram[addr - 0xC000],
            0xD000..=0xDFFF => self.internal_ram[addr - 0xC000 + 0x1000 * (self.wram_bank - 1)],
            0xE000..=0xFDFF => self.internal_ram[addr - 0xE000],
            0xFE00..=0xFE9F => self.oam[addr - 0xFE00],
            0xFF69 => self.bg_color_ram[self.io_registers[0x69] as usize],
            0xFF6B => self.obj_color_ram[self.io_registers[0x6B] as usize],
            0xFF00..=0xFF4B => self.io_registers[addr - 0xFF00],
    ...
    pub fn write_memory(&mut self, addr: impl Into<usize>, val: u8, source: RequestSource) {
        ...
        match addr {
            ...
            0x8000..=0x9FFF => {
                if self.ppu_mode != 3 || source == RequestSource::PPU {
                    if self.vram_bank == 0 {
                        self.vram_0[addr - 0x8000] = val;
                    } else {
                        self.vram_1[addr - 0x8000] = val;
                    }
                }
            }
            0xA000..=0xBFFF => match self.cartridge_type {
                ...
                CartType::Mbc1 | CartType::Mbc3 => {
                    if !(self.available_ram_banks == 0 || !self.ram_enable || self.memory_mode == 1)
                    {
                        self.external_ram[addr - 0xA000 + 0x2000 * self.ram_bank] = val;
                    }
                }
                CartType::Mbc2 => {
                    if self.ram_enable {
                        self.external_ram[(addr - 0xA000) % 0x200] = val;
                    }
                }
                CartType::Mbc5 => {
                    if self.ram_enable {
            ...
            0xC000..=0xCFFF => self.internal_ram[addr - 0xC000] = val,
            0xD000..=0xDFFF => {
                self.internal_ram[addr - 0xC000 + 0x1000 * (self.wram_bank - 1)] = val;
            }
            ...
            0xFF41 => {
                if source == RequestSource::PPU {
                    self.io_registers[0x41] = val;
                } else {
                    self.io_registers[0x41] &= 0b0000111;
                    self.io_registers[0x41] |= val & 0b1111000
                }
            }
            0xFF46 => {
            ...
            0xFF4F => {
                ...
                self.io_registers[0x4F] = 0b11111110 | (val & 1);
            }
            0xFF50 => {
                if self.in_boot_rom {
            ...
            0xFF68 => {
                self.bg_color_inc = (val >> 7) == 1;
                self.io_registers[0x69] = val & 0x3F;
                self.io_registers[0x68] = val;
            }
            0xFF69 => {
                self.bg_color_ram[self.io_registers[0x69] as usize] = val;
                if self.bg_color_inc {
                    self.io_registers[0x69] += 1;
                }
            }
            0xFF6A => {
                self.obj_color_inc = (val >> 7) == 1;
                self.io_registers[0x6B] = val & 0x3F;
                self.io_registers[0x6A] = val;
            }
            0xFF6B => {
                self.obj_color_ram[self.io_registers[0x6B] as usize] = val;
                if self.obj_color_inc {
                    self.io_registers[0x6B] += 1;
                }
            }
            0xFF70 => {
                if self.cgb {
                    self.wram_bank = (val & 0b111) as usize;
                    if self.wram_bank == 0 {
                        self.wram_bank += 1;
                    }
                }
    ...
    fn load_boot_rom(&mut self) {
        self.hold_mem[..256].copy_from_slice(&self.rom[..256]);
        if self.cgb {
            self.hold_mem[256..2048].copy_from_slice(&self.rom[0x200..0x900]);
            self.rom[..256].copy_from_slice(&CGB_BOOTROM_1);
            self.rom[0x200..0x900].copy_from_slice(&CGB_BOOTROM_2);
        } else {
            self.rom[..256].copy_from_slice(&DMG_BOOTROM);
        }
    }
    fn unload_boot_rom(&mut self) {
        self.rom[..256].copy_from_slice(&self.hold_mem[..256]);
        if self.cgb {
            self.rom[0x200..0x900].copy_from_slice(&self.hold_mem[256..2048]);
        }
    }
    ...
    fn dma_transfer(&mut self, reg: usize) {
        ...
        match reg >> 4 {
            ...
            0x8..=0x9 => {
                ...
                let adjusted_end_address = adjusted_start_address + 0xA0;
                self.oam
                    .copy_from_slice(&self.vram_0[adjusted_start_address..adjusted_end_address]);
            }
            0xA..=0xB => {
    ...
    pub fn get_bg_rbg(&self, palette: u8) -> [[u8; 4]; 4] {
        ...
        for i in 0..4 {
            ...
            index += 2;
            out[i] = [red * 8, green * 8, blue * 8, 0xFF]
        }
        out
    }
...
impl GameBoyEmulator {
    pub fn load_rom(&mut self, path: &Path) {
        ...
        self.mem_unit.available_rom_banks =
            1 << (self.mem_unit.get_memory(ROM_BANK_ADDR, SOURCE) + 1);
        self.mem_unit.available_ram_banks = match self.mem_unit.get_memory(RAM_BANK_ADDR, SOURCE) {
            0 => 0,
            2 => 1,
            3 => 4,
            4 => 16,
        ...
        self.mem_unit.rom_bank_bits =
            self.mem_unit.available_rom_banks.trailing_zeros() as usize + 1;
        self.cgb = (self.mem_unit.rom[0x143] >> 7) == 1;
        self.mem_unit.cgb = self.cgb;
        self.mem_unit.load_boot_rom();
    }
